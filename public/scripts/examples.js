define(function() {
  ...
  Events.bind = function() {
    ...
    $('#e4-buttonConnect').click(function(e) {
      e.preventDefault();

      console.log('firing up Noduino.');
      
      $('#e4-exampleConnection .alert').addClass('hide');    
      $('#e4-exampleConnection .alert-info').removeClass('hide');
      $('#e4-exampleConnection .alert-info').html('Trying to connect to your Arduino with ex 4 …');      
      require(['example-4'], function(example) {
        example.handle();
      });      
    });    

    $('#e5-buttonConnect').click(function(e) {
      e.preventDefault();

      console.log('firing up Johnny Five.');
      
      $('#e5-exampleConnection .alert').addClass('hide');    
      $('#e5exampleConnection .alert-info').removeClass('hide');
      $('#e5-exampleConnection .alert-info').html('Trying to connect to your Arduino with ex 5 …');      
      require(['example-5'], function(example) {
        example.handle();
      });      
    }); 
